...
import { ArrowLeft, Search as SearchIcon, BookOpen, FlaskConical, Briefcase, Lightbulb } from 'lucide-react';

type ResultType = 'definition' | 'formula' | 'case' | 'fs_app';
  ...
  const allResults = useMemo(() => {
    ...
    (embaData.definitions || []).forEach((d: any) => {
      if (d.term.toLowerCase().includes(q) || d.definition.toLowerCase().includes(q)) {
        results.push({ type: 'definition', id: d.id, title: d.term, content: d.definition, discipline: d.discipline });
      }
    });
    ...
    (embaData.formulas || []).forEach((f: any) => {
      if (f.name.toLowerCase().includes(q) || f.formula.toLowerCase().includes(q) || f.variables.some((v: string) => v.toLowerCase().includes(q))) {
        results.push({ type: 'formula', id: f.id, title: f.name, content: f.formula, discipline: f.discipline });
      }
    });

    (embaData.cases || []).forEach((c: any) => {
      if (c.title.toLowerCase().includes(q) || c.content.toLowerCase().includes(q)) {
        results.push({ type: 'case', id: c.id, title: c.title, content: c.content, discipline: c.discipline });
      }
    });

    (embaData.fs_applications || []).forEach((a: any) => {
      if (a.title.toLowerCase().includes(q) || a.content.toLowerCase().includes(q)) {
        results.push({ type: 'fs_app', id: a.id, title: a.title, content: a.content, discipline: a.discipline });
      }
    });
          ...
          <div className="px-6 lg:px-10 py-4">
            ...
            {query.length >= 2 && (
              <div className="flex items-center gap-1.5 mt-3 overflow-x-auto pb-1">
                {([['all', 'All'], ['definition', 'Definitions'], ['formula', 'Formulas'], ['case', 'Cases'], ['fs_app', 'FS Apps']] as [string, string][]).map(([key, label]) => (
                  <button
                    key={key}
          ...
          {query.length < 2 ? (
            ...
            <div className="space-y-2" role="list" aria-label={`${filtered.length} search results`}>
              {filtered.slice(0, 100).map((r, i) => {
                const color = DISCIPLINE_COLORS[r.discipline] || 'var(--primary)';
                return (
                  <motion.div
                    key={`${r.type}-${r.id}`}
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded text-primary-foreground" style={{ background: color }}>
                        {r.discipline}
                      </span>
                    </div>
                    {r.type === 'definition' ? (
                      <Link href={`/discipline/${slugify(r.discipline)}`}>
                        <h3 className="text-sm font-semibold mb-1 hover:text-primary transition-colors" style={{ fontFamily: 'var(--font-display)' }}>
                          {r.title}
                        </h3>
                      </Link>
                    ) : (
                      <h3 className="text-sm font-semibold mb-1" style={{ fontFamily: 'var(--font-display)' }}>{r.title}</h3>
                    )}
